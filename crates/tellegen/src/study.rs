...
pub struct Study {
    ...
    /// lets a save replace only the value while carrying valid producer,
    /// source descriptors, diagnostics, history, extensions, and source map.
...
impl Study {
    ...
    pub fn from_module(
        ...
        let module_json = write_module(&dynamic).map_err(|e| e.to_string())?;
    ...
    fn from_dynamic_module(
        ...
        let module_json = write_module(&module).map_err(|e| e.to_string())?;
    ...
    pub fn save_module(&self) -> Result<String, String> {
        ...
        write_module(&module).map_err(|e| e.to_string())
    }
    ...
    /// `model-json`, ...). Any fidelity the target cannot carry rides back in
    /// `warnings` as `CODE: message` lines so the caller can surface it — for
    /// `model-json` those are the fields JSON wrote as `null`, which is what
    /// stops the document reading back.
    pub fn export(&self, format: &str) -> Result<ExportedCase, String> {
        ...
        match target {
            ExportTarget::Case(format) => {
                let conversion =
                    powerio::write_network(&balanced, format).map_err(|e| e.to_string())?;
                Ok(ExportedCase {
                    warnings: conversion.rendered_diagnostics(),
...
/// Where an export lands: a case format writer, or powerio's own model JSON.
///
...
mod tests {
    ...
    fn module_json(network: BalancedNetwork) -> String {
        ...
        write_module(&module).expect("write module")
    }
    ...
    fn save_module_retains_existing_module_records_and_uses_unique_history_ids() {
        ...
        let mut stored: Value =
            serde_json::from_str(&write_module(&dynamic).expect("write source module"))
                .expect("stored JSON");
        ...
        let module = powerio::try_into_typed(module).expect("typed source module");
    ...
    fn a_saved_module_reads_back_as_a_fresh_base_at_the_committed_point() {
        ...
        let module: PioModule<BalancedNetwork> = try_into_typed(module).unwrap();
