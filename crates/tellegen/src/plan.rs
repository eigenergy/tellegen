...
impl CapacityPlanExecution {
    ...
    ) -> Result<(CapacityPlanOutcome, powerio::DcOpfSolution), String> {
...
mod tests {
    ...
    #[test]
    fn weighted_lmp_gradient_matches_finite_differences_across_an_active_set_change() {
        let weights: Vec<(usize, f64)> = vec![(0, 0.25), (1, 1.0), (2, 0.5)];
        let binding = congested_case3();
        let mut slack = binding.clone();
        // The natural flow is about 39.8 MW. These two ratings sit on opposite
        // sides of the line limit active set change while leaving a 0.5 MW
        // finite-difference stencil inside each smooth region.
        slack.fmax[0] = 0.45;

        for (label, dc, line_is_binding) in [("binding", binding, true), ("slack", slack, false)] {
            let sol = dc_opf_cancellable(&dc, None).expect("base solve");
            let line_dual = sol.lam_ub[0] + sol.lam_lb[0];
            if line_is_binding {
                assert!(line_dual > 1e-3, "binding side dual is {line_dual}");
            } else {
                assert!(line_dual < 1e-5, "slack side dual is {line_dual}");
            }
            ...
            // The functional adjoint must also agree with the full matrix
            // contraction in both active sets.
            ...
            for (e, &g) in grad.iter().enumerate() {
                ...
                assert!(
                    ...
                    "{label} branch {e}: adjoint {g} vs contracted {contracted}"
                );
            }

            // Central finite differences on the exact solve, in MW. The
            // comparison is made at an operating point on each side of the
            // active set change, not with a stencil that straddles the kink.
            let h_mw = 0.5;
            for (e, &g) in grad.iter().enumerate() {
                let mut up = dc.clone();
                ...
                assert!(
                    ...
                    "{label} branch {e}: adjoint {g} vs finite difference {fd}"
                );
            }
        }
    }
