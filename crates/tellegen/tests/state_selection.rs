...
use powerio::select::{
    select_state, state_inventory, SelectedState, StateInventory, StateSelector,
};
...
fn a_selected_series_state_solves_without_an_exported_module() {
    ...
    let StateInventory::TimePoints(points) = state_inventory(&value).expect("inventory") else {
    ...
    for position in 0..3 {
        ...
        let instance =
            powerio_prob::DcOpfInstance::from_network(network.clone()).expect("problem instance");
        let solution =
    ...
    let refused = state_inventory(&static_value).unwrap_err();
