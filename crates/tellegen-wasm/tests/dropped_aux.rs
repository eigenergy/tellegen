...
use tellegen::geo::{apply_aux_substation_locations, network_coords, spread_stacks};
...
fn dropped_aux_parses_solves_and_differentiates() {
    ...
    // 2. Coordinates. The aux carries substation coordinates in its retained
    //    source text. Browser ingestion materializes that join on the retained
    //    module value, so the view no longer depends on replaying source bytes.
    let mut coords = network_coords(&net, None);
