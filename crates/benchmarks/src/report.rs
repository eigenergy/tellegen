//! Result recording for `docs/src/benchmark-results.md`: JSON and CSV artifacts,
//! one row per `(case, variant)`, plus a markdown snapshot the mdBook renders
//! without the corpus present. Each artifact records its toolchain provenance.
...
pub fn render_markdown(prov: &Provenance, records: &[Record]) -> String {
    ...
    s.push_str(
        "Generated by the `benchmarks` harness over PGLib-OPF v23.07. Regenerate with \
         ...
         The provenance table records the toolchain used for this run.\n\n",
    );
    ...
    s.push_str(
        "Tellegen's objective against the published value, per formulation: the DC OPF \
         (constant cost included) vs the published DC, and the SOCWR relaxation (a lower bound on \
         AC) vs the published AC. `Δgap` is Tellegen's SOC gap minus the published SOC gap. \
         The **reproduces** column is `DC·SOC`: ✓ \
    ...
    s.push_str(
        "Per size band. `adj−fwd` is the largest observed difference between the two \
         implicit solve directions. `FdClean` and `coupled` are central difference \
         relative errors per parity class, shown as median/worst. Coupled cells contain \
         reactive quantities, squared voltage, or conductance. A central difference that \
         crosses an active set boundary measures two local regimes, so the table keeps both \
         median and worst values. Columns below the stated numerical floor are skipped.\n\n",
    );
    s.push_str(
...
fn reproduction_summary(records: &[Record]) -> String {
    ...
    let mut s = String::new();
    s.push_str(
        "Corpus summary of the per case marks in the table below. \"Reproduced\" is an \
