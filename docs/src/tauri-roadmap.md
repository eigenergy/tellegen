...
- **The full nonlinear AC OPF.** The browser solves DC OPF, AC power flow, and
  the SOCWR relaxation. The engine now has a non-default, development-only
  native AC OPF backend; a Tauri product would expose it on the device after
  its solver, licensing, and distribution gates are complete.
...
- `crates/tellegen-tauri`: a workspace member that holds a native `Study` and
  exposes it through `#[tauri::command]`. It would provide the approved
  transport for the engine's existing opt-in AC OPF backend.
