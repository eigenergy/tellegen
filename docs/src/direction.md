...
[Architecture](architecture.md) records the current boundary as shipped; this
note records the intent the boundary serves.
...
sensitivities. PowerDiff worked out the sensitivity column that makes this
...
with no server. That move has landed (issue #2), and it is what turns tellegen
...
against powerio and compiles to both wasm and native (done). The next step is to
package the Svelte code with `@sveltejs/package`: export the map components, the
parsed-case data model, the layer accessor and theme props, and the wasm parser
and solver as standalone entry points; peer-depend deck.gl and maplibre; ship the
wasm through `?url`. The demo becomes the reference consumer. This is the
highest-leverage step toward "framework."
...
engines that cannot run the wasm path. The remaining work here is hardening:
close the Safari sensitivity gap (issue #8), prebake the bundled cases as static
assets, and make the no-backend deployment the default.
