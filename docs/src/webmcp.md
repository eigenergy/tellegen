...
Tellegen exposes the solved case in the current browser tab through nine
structured tools. The tools read and update the same state shown in the
interface and run the same WebAssembly solver.
...
| `preview_case_update` | Solve proposed demand or rating edits on cloned state. |
...
| `propose_capacity_plan` | Search capacity increases with implicit derivatives and exact trial solves, then stage an unapplied proposal. |
| `apply_capacity_plan` | Apply an approved proposal after checking its session, case, and revision. |

The first seven tools register for the page lifecycle. `inspect_case` reports
when no solvable case is active; the other six require one.
`propose_capacity_plan` requires DC OPF. `apply_capacity_plan` registers only
while a staged proposal matches the current state.
...
orders capacity increases by that local direction, and checks each trial with
an exact solve. It recomputes the direction after an accepted active set
change. The result is a bounded heuristic proposal. It includes the first
order prediction and exact result for each trial and does not claim global
optimality.
...
the materialized network as a PowerIO module. Saving an exact result writes a
PowerIO solution module containing the amended instance that was solved.
Tellegen defines no study or experiment document schema, and descriptive
PowerIO history is not a replay program.
...
Playwright uses a standards shaped test host. Before release, invoke the tools
through native WebMCP in the in-app browser as a separate bridge check.
