...
place it on the map. Dropped JSON is content routed. A stored
`powerio.module` document opens by its value kind: a balanced network becomes
a case, while a multiconductor network opens in the viewer. Model and
transmission JSON open as balanced cases, and BMOPF or
...
fresh runtime study at the saved point; descriptive edit history is not
replayed. The case panel can also download the current layout as a `.geo.json`
layer that PowerIO and Tellegen read back.
