...
export function createTellegenTools(
  ...
  return [
    ...
    },
    {
      ...
      description:
        "Focus a bus or branch in the tellegen interface and load its exact sensitivity view. The returned result reflects the interface after the selection has settled.",
      inputSchema: objectSchema(
        {
          case_id: {
            type: "string",
...
export function createTellegenPlanningTools(
  ...
  const proposeCapacityPlan: TellegenToolDefinition = {
    ...
    description:
      "Use implicit derivatives to guide a bounded search over branch capacity increases. Exact trial solves verify the staged proposal. The electrical case stays unchanged.",
    inputSchema: objectSchema(
      {
        case_id: {
          type: "string",
          ...
          ...objectSchema(
            {
              ...
              weights: {
                ...
                minItems: 1,
                maxItems: 16,
                description: "Weighted LMP terms minimized by the search.",
                items: objectSchema(
                  {
                    bus_id: {
                      type: "string",
